...
namespace ESPEJIC.gestion_de_stage
{
    public partial class la_liste_1ere : UserControl
    {
        ...
        private void la_liste_1ere_Load(object sender, EventArgs e)
        {
            FillInformationEcole();
            cn.Open();
            cmd = new SqlCommand("Select Stage.CIN_Stagaire,Stagaire.Nom_stagaire,Stagaire.Pre_stagaire,Stagaire.Type_stagaire,Nom_Entr,Addrs_Ent,Stage.Tele,Date_d,Date_f, DATEDIFF(MONTH, Date_d, Date_f) from Stage inner join Stagaire on Stage.CIN_Stagaire = Stagaire.CIN_Stagaire where Type_stagaire='1er année'", cn);
